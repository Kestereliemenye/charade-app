import * as Icons from "phosphor-react-native";

export const DECK_DATA = [
  {
    id: "1",
    title: "Nollywood Stars",
    icon: Icons.FilmSlate,
    bgImage: require("../assets/images/nollywoodBg.png"),
    lobbyImg: require("../assets/images/nollywoodLobbyImg.png"),
    intro:
      "Lights, Camera, Action! Dive into the heart of African cinema with the ultimate Nollywood challenge. Think you know your movie trivia? Prove it on the screen.",
  },
  { id: "2", title: "Afrobeats", icon: Icons.MusicNotes },
  { id: "3", title: "Lagos Life", icon: Icons.Buildings },
  { id: "4", title: "Street Food", icon: Icons.Hamburger },
  { id: "5", title: "Pidgin", icon: Icons.ChatCircleText },
  { id: "6", title: "Football", icon: Icons.SoccerBall },
  { id: "7", title: "Market", icon: Icons.ShoppingBag },
  { id: "8", title: "Party", icon: Icons.Confetti },
  { id: "9", title: "Tech", icon: Icons.Desktop },
  { id: "10", title: "Classic", icon: Icons.MaskHappy },
  { id: "11", title: "History", icon: Icons.BookOpenText },
  { id: "12", title: "Travel", icon: Icons.AirplaneTilt },
  { id: "13", title: "Politics", icon: Icons.Gavel },
  { id: "14", title: "Art", icon: Icons.PaintBrush },
  { id: "15", title: "Science", icon: Icons.Flask },
  { id: "16", title: "Relationships", icon: Icons.Heart },
];
